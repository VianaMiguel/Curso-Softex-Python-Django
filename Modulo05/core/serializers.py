...
from django.contrib.auth.models import User, Group  # Exercicio 1 apostila 6
...
class TarefaSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):  # exercicio 1 apostila 6
        if data.get("prioridade") == "alta" and data.get("concluida") is True:
            raise serializers.ValidationError(
                {
                    "concluida": "Tarefas com prioridade alta não podem ser criadas já concluídas."
                }
            )
        return data
    
    def validate_titulo(self, value):  # NOVO método exercicio 1 apostila 6
        # Regra 1: mínimo de 3 caracteres
        if len(value) < 3:
            raise serializers.ValidationError("O título deve ter pelo menos 3 caracteres.")
        # Regra 2: apenas letras
        if not value.isalpha():
            raise serializers.ValidationError("O título deve conter apenas letras.")
