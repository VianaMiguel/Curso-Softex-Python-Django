...
class TarefaAPITest(APITestCase):
    """ Testes de Integração para os endpoints de Tarefa. Simula o ciclo completo: Request -> URL -> View -> Serializer -> DB -> Response """
    def setUp(self):
        ...
        self.tarefa_user1 = Tarefa.objects.create( user=self.user1, titulo='Tarefa do User 1', concluida=False )
        self.tarefa_user2 = Tarefa.objects.create( user=self.user2, titulo='Tarefa do User 2', concluida=True )

# ------------------ Helpers ------------------ 
    def obter_token(self, username, password): 
        """ Método auxiliar para realizar login e obter o token JWT. Evita repetição de código nos testes. """ 
